...
import com.pindroid.util.Base64;
...
public class OauthUtilities {
	public static HttpGet signRequest(HttpGet request, TreeMap<String, String> params, String authtoken,
		...
		for(String key : params.keySet()){
			...
			sb.append(URLEncoder.encode(params.get(key)).replace("+", "%2B").replace("%7C", "%257C").replace("%7B", "%257B").replace("%7D", "%257D").replace("%26", "%2526").replace("%2C", "%252C"));
		}
